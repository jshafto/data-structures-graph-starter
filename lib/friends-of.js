...
function friendsOfR(adjacencyList, name,  maxDistance, visited = new Set(),  currentDistance = 0) {
  if (Object.keys(adjacencyList).length===0 || !Object.keys(adjacencyList).includes(name)) return undefined;
  if (name in visited) return Array.from(visited).slice(1);
  visited.add(name);
  if (currentDistance >= maxDistance) return Array.from(visited).slice(1);
  adjacencyList[name].forEach(el => {
    friendsOfR( adjacencyList, el, maxDistance, visited, currentDistance+1);
  })
